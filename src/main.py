...
class BirdDetector:
    def __init__(self, persistence_frames: int = 3, position_tolerance: int = 60):
        ...
        self.active_tracks = []   # list of dicts: {"center", "count", "area", "rect"}
    ...
    def _update_tracks(self, current_candidates):
        ...
        - Continue tracks that match
        - Start new tracks for unmatched detections
        - Drop tracks that were not matched this frame
        Returns list of confirmed candidates.
        """
        confirmed = []
        new_tracks = []
        matched_indices = set()
        ...
        for center, area, contour, rect in current_candidates:
            match_idx = self._find_best_match(center, matched_indices)

            if match_idx is not None:
                # Continue existing track
                track = self.active_tracks[match_idx]
                track["center"] = center
                track["count"] += 1
                track["area"] = area
                track["rect"] = rect
                ...
                if track["count"] >= self.persistence_frames:
        ...
        # Keep only matched tracks + new ones
        self.active_tracks = [
            track for i, track in enumerate(self.active_tracks)
    ...
    def _find_best_match(self, center, already_matched: set) -> int | None:
        """Return index of the closest unmatched track within tolerance, or None."""
        cx, cy = center
        best_idx = None
        best_dist = float("inf")
    ...
    def _draw_detections(self, output, confirmed_candidates):
        """Draw green boxes and labels on the output image."""
